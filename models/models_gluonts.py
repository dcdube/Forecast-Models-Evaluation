...
def train_gluonts_model(y_df, model_name, save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs):
...
def train_all_models(dataset_name, dataset, start_dt, end_dt, save_dir, model_name, model_class, sampling_rate, epochs):
    ...
    if dataset_name == "belgium":
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, pv_mae, pv_rmse = train_gluonts_model(pv_data, f"PV_house_{house}", save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs)
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, b_mae, b_rmse = train_gluonts_model(battery_data, f"BESS_house_{house}", save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs)
        ...
        _, load_mae, load_rmse = train_gluonts_model(germany_data, "germany_load", save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs)
        ...
        _, load_mae, load_rmse = train_gluonts_model(london_data, "london_load", save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs)
        ...
        for customer_id in [8, 9, 43]:
            ...
            _, c_mae, c_rmse = train_gluonts_model(
                cust_data, f"zonnedael_customer_{customer_id}", save_dir, model_class, sampling_rate, forecast_horizon, freq, epochs
            )
...
def paper_forecasting_train(dataset_name, dataset, sampling_rates, models, runs, epochs, results_dir=PROJECT_ROOT / "results"):
        ...
        for model_name, model_class in models.items():
            ...
            for run_num in range(1, runs + 1):
                try:
                    ...
                    train_all_models(dataset_name, dataset, start_dt, end_dt, save_dir, model_name, model_class, sampling_rate, epochs)
                    gc.collect()
                    if mx.context.num_gpus() > 0:
                        mx.nd.waitall()
...
if __name__ == "__main__":
    ...
    paper_forecasting_train(args.dataset, dataset, args.sampling_rates, selected_models, args.runs, args.epochs, args.results_dir)

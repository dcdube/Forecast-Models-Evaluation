...
def neural_forecast_model(y_df, model_name, save_dir, freq, forecast_horizon, sampling_rate, epochs, nf_model_name="TimesNet", NFmodel=None):
...
def train_all_models(dataset_name, dataset, start_dt, end_dt, save_dir, freq, forecast_horizon, sampling_rate, epochs, nf_model_name, NFmodel):
    ...
    if dataset_name == "belgium":
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, pv_mae, pv_rmse = neural_forecast_model(
                pv_data, f"PV_house_{house}", save_dir, freq, forecast_horizon, sampling_rate, epochs, nf_model_name, NFmodel
            )
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, battery_mae, battery_rmse = neural_forecast_model(battery_data, f"BESS_house_{house}", save_dir, freq, forecast_horizon,
                                                                 sampling_rate, epochs, nf_model_name, NFmodel)
        ...
        _, load_mae, load_rmse = neural_forecast_model(
            germany_data, "germany_load", save_dir, freq, forecast_horizon, sampling_rate, epochs, nf_model_name, NFmodel
        )
        ...
        _, load_mae, load_rmse = neural_forecast_model(
            london_data, "london_load", save_dir, freq, forecast_horizon, sampling_rate, epochs, nf_model_name, NFmodel
        )
...
def paper_forecasting_train(dataset_name, dataset, run_num, sampling_rate, epochs, models, results_dir=PROJECT_ROOT / "results"):
    ...
    for model_name, model_class in models.items():
        try:
            ...
            train_all_models(
                ...
                sampling_rate, epochs, nf_model_name=model_name, NFmodel=model_class
            )
...
if __name__ == "__main__":
    ...
    for sampling_rate in args.sampling_rates:
        ...
        for run_num in range(1, args.runs + 1):
            paper_forecasting_train(args.dataset, dataset, run_num, sampling_rate, args.epochs, selected_models, args.results_dir)

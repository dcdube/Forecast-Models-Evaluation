...
def generic_forecast_model(y_df, model_name, save_dir, freq, forecast_horizon, nf_model_name="NaiveDrift", NFmodel=None):
    ...
    logging.info(f"{model_name} ({nf_model_name}) - MAE: {mae:.4f}, RMSE: {rmse:.4f}, MAPE: {mape:.4f}, R2: {r2:.4f}")
...
def train_all_models(start_dt, end_dt, save_dir, freq, forecast_horizon, nf_model_name, NFmodel):
    ...
    if selected_dataset == "belgium":
        logging.info("Training load consumption model")
        load_data = dataset_belgium.get_inputs_for_load(start_dt, end_dt)
        _, load_mae, load_rmse, load_mape, load_r2 = train_load_model(load_data, save_dir, freq, forecast_horizon, nf_model_name, NFmodel)
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, pv_mae, pv_rmse, pv_mape, pv_r2 = train_pv_model(pv_data, save_dir, house, freq, forecast_horizon, nf_model_name, NFmodel)
        ...
        for house in [1, 2, 3, 4]:
            ...
            _, battery_mae, battery_rmse, battery_mape, battery_r2 = train_battery_model(battery_data, save_dir, house, freq, forecast_horizon, nf_model_name, NFmodel)
        ...
        _, load_mae, load_rmse, load_mape, load_r2 = train_london_model(london_data, save_dir, freq, forecast_horizon, nf_model_name, NFmodel)
        ...
        for customer_id in [8, 9, 43]:
            ...
            _, cust_mae, cust_rmse, cust_mape, cust_r2 = train_zonnedael_model(customer_id, customer_data, save_dir, freq, forecast_horizon, nf_model_name, NFmodel)
